...
SELECT emp_name, emp_id, gender, dept_id, hire_date, salary,
       RANK() OVER(PARTITION BY gender ORDER BY hire_date ASC) AS rnk
    FROM employee
    WHERE retire_date IS NULL
...
SELECT ROW_NUMBER() OVER(ORDER BY hire_date ASC) AS num,
       emp_name, emp_id, gender, dept_id, hire_date, salary
    FROM employee
    WHERE retire_date IS NULL;
...
SELECT ROW_NUMBER() OVER(PARTITION BY dept_id ORDER BY hire_date ASC) AS num,
       emp_name, emp_id, gender, dept_id, hire_date, salary
    FROM employee
    WHERE retire_date IS NULL;
...
SELECT emp_name, emp_id, gender, dept_id, hire_date, COALESCE(salary, 5000) AS salary,
       DENSE_RANK() OVER(ORDER BY COALESCE(salary, 5000) DESC) AS rnk
    FROM employee
    WHERE retire_date IS NULL;
