...
SELECT emp_id,
       COUNT(*) AS cnt_duration,
       SUM(duration) AS tot_duration,
       GROUP_CONCAT(duration ORDER BY begin_date DESC SEPARATOR ',') AS duration
    FROM vacation
    GROUP BY emp_id;
...
SELECT v.emp_id, e.emp_name, e.dept_id, v.begin_date, v.duration,
       SUM(duration) OVER(PARTITION BY emp_id ORDER BY begin_date ASC) AS cum_duration
    FROM vacation AS v
...
SELECT v.emp_id, e.emp_name, e.dept_id, v.begin_date, v.duration,
       FIRST_VALUE(v.begin_date) OVER(PARTITION BY v.emp_id 
                                      ORDER BY v.begin_date ASC
                                      ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS first_begin_date,
       LAST_VALUE(v.begin_date)  OVER(PARTITION BY v.emp_id
                                      ORDER BY v.begin_date ASC
                                      ROWS BETWEEN CURRENT ROW AND UNBOUNDED FOLLOWING) AS last_begin_date
    FROM vacation AS v
    INNER JOIN employee AS e ON v.emp_id = e.emp_id;
...
SELECT v.emp_id, e.emp_name, e.dept_id, v.begin_date, v.duration,
       LAG(v.begin_date, 1) OVER(PARTITION BY emp_id ORDER BY v.begin_date ASC) AS prev_begin_date,
       DATEDIFF(v.begin_date, 
                LAG(v.begin_date, 1) OVER(PARTITION BY emp_id 
                                          ORDER BY v.begin_date ASC)) AS date_diff
    FROM vacation AS v
    INNER JOIN employee AS e ON v.emp_id = e.emp_id;
