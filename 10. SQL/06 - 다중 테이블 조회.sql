...
SELECT emp_id, emp_name, dept_id, hire_date, retire_date, phone, email
	FROM employee AS e
	WHERE retire_date = (SELECT MAX(retire_date) FROM employee);
   
-- Q) 강우동(S0003)보다 급여를 많이 받는 직원 정보 조회
SELECT emp_id, emp_name, dept_id, hire_date, retire_date, phone, email, salary
	FROM employee AS e
